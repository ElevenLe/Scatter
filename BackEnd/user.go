...
func addUser(user *User) (bool, error) {
	...
	// see search length is 0, then no one call. Otherwise there is user exits
...
// one is pointer: responseWriter is interface type, which do not have pointer or not
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	// first parameter is header, second parameter is payload
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"username": user.Username,
		// here is token expire time. Here is set as 24 hours later
